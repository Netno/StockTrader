...
export default async function HistoryPage() {
  ...
  const winRate =
    trades.length > 0 ? Math.round((winners.length / trades.length) * 100) : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 text-center">
          ...
          <p
            className={`text-2xl font-bold ${totalPnlKr >= 0 ? "text-green-400" : "text-red-400"}`}
          >
            {totalPnlKr >= 0 ? "+" : ""}
            {totalPnlKr.toFixed(0)} kr
          </p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 text-center">
              ...
              return (
                <tr key={t.id} className="hover:bg-gray-800/30 transition">
                  ...
                  <td className="p-4 text-right">
                    ...
                    <p
                      className={`text-xs ${pnlPct >= 0 ? "text-green-400" : "text-red-400"}`}
                    >
                      {pnlPct >= 0 ? "+" : ""}
                      {pnlPct.toFixed(1)}%
                    </p>
