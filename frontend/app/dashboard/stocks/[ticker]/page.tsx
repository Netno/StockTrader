...
export default async function StockDetailPage({
  ...
  const activeTab: Tab = (
    TABS.includes(rawTab as Tab) ? rawTab : "indikatorer"
  ) as Tab;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        {live?.buy_score !== undefined && (
          ...
          >
            Köpsignal: {live.buy_score}p{" "}
            {live.buy_score >= 60 ? "— AKTIV" : "/ 60p"}
          </div>
        )}
      </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
                  ...
                  },
                  {
                    ...
                    color: ind.macd >= 0 ? "text-green-400" : "text-red-400",
                  },
                  {
                    label: "MA50",
                    value: ind.ma50?.toFixed(2),
                    color: price > ind.ma50 ? "text-green-400" : "text-red-400",
                  },
                  {
                    label: "MA200",
                    value: ind.ma200?.toFixed(2),
                    color:
                      price > ind.ma200 ? "text-green-400" : "text-red-400",
                  },
                  {
                    label: "EMA20",
                  ...
                  {
                    label: "Bollinger over",
                    value: ind.bollinger_upper?.toFixed(2),
                    color: "text-white",
                  },
                  {
                    label: "Bollinger under",
                    value: ind.bollinger_lower?.toFixed(2),
                    color: "text-white",
                  },
                  {
                    label: "ATR",
                  ...
                  {
                    label: "Volym x",
                    value: ind.volume_ratio ? `${ind.volume_ratio}x` : null,
                    color:
                      ind.volume_ratio >= 1.5 ? "text-green-400" : "text-white",
                  },
                  {
                    label: "Stop-loss",
                    value: live?.stop_loss?.toFixed(2),
                    color: "text-red-400",
                  },
                  {
                    label: "Take-profit",
                    value: live?.take_profit?.toFixed(2),
                    color: "text-green-400",
                  },
                  {
                    label: "Datapunkter",
                    value: live?.data_points,
                    color: "text-gray-400",
                  },
              ...
              {live?.buy_reasons?.length > 0 && (
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                  <p className="text-xs text-gray-500 mb-3">
                    Aktiva köpsignals-kriterier
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {live.buy_reasons.map((r: string, i: number) => (
                      <span
                        key={i}
                        className="text-xs bg-blue-500/10 text-blue-400 px-2.5 py-1 rounded-full border border-blue-500/20"
                      >
                        {r}
                      </span>
                    ))}
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {signals.map((s: any) => (
                      <tr key={s.id} className="hover:bg-gray-800/30">
                        ...
                        <td className="p-4">
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs font-semibold ${s.signal_type === "BUY" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
                          >
                        ...
                        <td className="p-4 text-xs">
                          <span className="text-red-400">
                            {s.stop_loss_price?.toFixed(2)}
                          </span>
                          <span className="text-gray-600 mx-1">/</span>
                          <span className="text-green-400">
                            {s.take_profit_price?.toFixed(2)}
                          </span>
                        </td>
                      </tr>
                    ))}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-4 flex-wrap">
                      ...
                      <div className="text-xs">
                        <span className="text-red-400">
                          SL {s.stop_loss_price?.toFixed(2) ?? "–"}
                        </span>
                        <span className="text-gray-600 mx-1">/</span>
                        <span className="text-green-400">
                          TP {s.take_profit_price?.toFixed(2) ?? "–"}
                        </span>
                      </div>
                    </div>
                  </div>
      ...
      {activeTab === "nyheter" && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl divide-y divide-gray-800">
          {news.length === 0 ? (
            <p className="p-6 text-gray-500 text-sm">
              Inga nyheter inhämtade för {ticker} än.
            </p>
          ) : (
            (news as any[]).map((item) => (
              <div
                key={item.id}
                className="p-4 flex items-start gap-3 hover:bg-gray-800/30 transition"
              >
                <span
      ...
      {activeTab === "notiser" && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl divide-y divide-gray-800">
          {notifications.length === 0 ? (
            <p className="p-6 text-gray-500 text-sm">
              Inga notiser skickade för {ticker} än.
            </p>
          ) : (
            (notifications as any[]).map((n) => (
              <div
                key={n.id}
                className="p-4 flex gap-3 hover:bg-gray-800/30 transition"
              >
                <div
                  className={`mt-0.5 w-7 h-7 rounded-full border flex items-center justify-center text-xs font-bold shrink-0 ${notifColor[n.type] ?? notifColor.info}`}
                >
                  {notifIcon[n.type] ?? "i"}
                </div>
                <div className="flex-1 min-w-0">
