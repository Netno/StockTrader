...
export default async function DashboardPage() {
  const [signals, positions, trades, summary, portfolioNotifs] =
  ...
  const notifData =
    portfolioNotifs.status === "fulfilled" ? portfolioNotifs.value : [];
  ...
  const totalPnlKr = tradesData.reduce(
    (s: number, t: any) => s + (t.pnl_kr ?? 0),
    0,
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          label="Investerat"
          value={`${invested.toFixed(0)} kr`}
          sub={`${openCount} position${openCount !== 1 ? "er" : ""}`}
        />
        ...
        <StatCard
          ...
          sub={
            totalTrades > 0
              ? `${Math.round(
                  (tradesData.filter((t: any) => (t.pnl_kr ?? 0) > 0).length /
                    totalTrades) *
                    100,
                )}% vinst`
              : undefined
        ...
        <div className="bg-amber-500/5 border border-amber-500/30 rounded-xl p-5 space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-amber-400">
              {pendingSignals.length} köpsignal
              {pendingSignals.length > 1 ? "er" : ""} väntar på bekräftelse
            </p>
            <Link
              ...
              <div className="flex items-center justify-between gap-4 flex-wrap">
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    {s.quantity} aktier &middot; ~
                    {(s.price * s.quantity)?.toFixed(0)} kr &middot; Score{" "}
                    {s.score}p
                  </p>
        ...
        <div className="xl:col-span-2 bg-gray-900 border border-gray-800 rounded-xl p-5">
          ...
          {openCount === 0 ? (
            <p className="text-gray-600 text-sm">
              Inga öppna positioner. Köpsignaler dyker upp här när agenten
              hittar möjligheter.
            </p>
          ) : (
            <div className="space-y-3">
              {Object.entries(positionsData as Record<string, any>).map(
                ([ticker, pos]) => {
                  const pnlColor =
                    pos.pnl_kr >= 0 ? "text-green-400" : "text-red-400";
                  return (
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="text-sm text-gray-300">
                            Ingatt {pos.price?.toFixed(2)} kr &times;{" "}
                            {pos.quantity} st
                          </p>
